...
**Perceptron** - **pros**: variety of complex processes can be solved with perceptrons - **cons**: user have to choose parameters(weight, bias) him/herself.
...
**Perceptron** and **Neural Network** looks similar. - The main difference is in **activate function**. - Perceptron uses **step function** as activate function. - Neural network uses **sigmoid, ReLU, tanh etc.**
...
**activation function** - $h(x)$ above is called activation function. - This convert *sum of all the input signals* to *output signals.*
...
The problem of linear function is here. - The combination of linear function is also linear. - As we saw at Chapter 2, we cannot solve complicated problems with one linear line.
...
There is no difference between row vector, and column vector. - They are treated same when multiplying
